...
function indekseGoreFenomen(fenomenler, index) {
  const fenomen = fenomenler[index];
  return `${index}. indekste bulunan fenomen: ${fenomen.profile}`;
}
...
function profilListesi(fenomenDizisi) {
  const yeniDizi = fenomenDizisi.slice();
  const isimler = [];

  for (let i = 0; i < yeniDizi.length; i++) {
    isimler.push(yeniDizi[i].profile);
  }
  return isimler;
}
...
function fenomenSil(fenomenDizisi, dizin) {
  const copyFenomenler = fenomenDizisi.slice();
  if (dizin >= 0 && dizin < copyFenomenler.length) {
    copyFenomenler.splice(dizin, 1);
  }
...
function enFenomenler(fenomenlerDizisi) {
  ...
  for (let i = 0; i < fenomenlerDizisi.length; i++) {
    if (fenomenlerDizisi[i].followers >= 100000000) {
      starlar.push(fenomenlerDizisi[i].profile);
    }
  }
  return starlar;
}
...
function fenomenGonderimSayisi(fenomenDizisi, profile) {
  for (let i = 0; i < fenomenDizisi.length; i++) {
    if (fenomenDizisi[i].profile === profile) {
...
function platformaGoreCokGonderiYapanFenomen(fenomenDizisi, platform) {
  let secilenKisiler = [];
  let postSayisi = [];

  for (let i = 0; i < fenomenDizisi.length; i++) {
    if (fenomenDizisi[i].platform === platform) {
      secilenKisiler.push(fenomenDizisi[i].profile);
      postSayisi.push(fenomenDizisi[i].posts);
    }
  }
  const max = Math.max(...postSayisi);
  const indexNumber = postSayisi.indexOf(max);
  return secilenKisiler[indexNumber];
}
